...
int main(int argc, char *argv[])
{
    ...
    if(atoi(argv[1])==0)
    {
        std::cout << "sswap,sstretch,sassignment,smultiply add,smultiply subtract,sinner dot product,";
        std::cout << "dswap,stretch,dassignment,dmultiply add,dmultiply subtract,dinner dot product,";
    }
    else
    {
        //fprintf(stdout,"Benchmark : BLAS1_von Franz\n");
        std::size_t BLAS1_N = atoi(argv[1]);
        std::cout << std::setprecision(3) << std::left << std::setw(8) << double(BLAS1_N) <<" ";
